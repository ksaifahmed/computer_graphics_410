...
void IntensifyPixel(int x, int y, double distance, bitmap_image &image, Color color, bool isSwap)
{
...
void colorImageGupta(bitmap_image &image, int x, int y, Color color, bool isSwap, int two_v_dx, double invDenom, double two_dx_invDenom)
{
    // for SMOOTH antialiasing
    IntensifyPixel(x, y, two_v_dx * invDenom, image, color, isSwap);
    IntensifyPixel(x, y + 1, two_dx_invDenom - two_v_dx * invDenom, image, color, isSwap);
    IntensifyPixel(x, y - 1, two_dx_invDenom + two_v_dx * invDenom, image, color, isSwap);
    IntensifyPixel(x, y + 2, 2*two_dx_invDenom - (two_v_dx * invDenom), image, color, isSwap);
    IntensifyPixel(x, y - 2, 2*two_dx_invDenom + (two_v_dx * invDenom), image, color, isSwap);
}
...
void WeightedAreaSamplingAntiAliased(Line line, bitmap_image &image, bool isLargeGradient, bool isGradientNegative, Color color)
{
    ...
    while (x < line.x1) {
        ...
        isGradientNegative
            ? isLargeGradient
                ? colorImageGupta(image, x, -y, color, isLargeGradient, two_v_dx, invDenom, two_dx_invDenom)
                : colorImageGupta(image, -x, y, color, isLargeGradient, two_v_dx, invDenom, two_dx_invDenom)
            : colorImageGupta(image, x, y, color, isLargeGradient, two_v_dx, invDenom, two_dx_invDenom);
    }
}
